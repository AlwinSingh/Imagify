...
def run_removeBg(user_img):
    ...
    # read image
    #im = cv2.imread(image_path) # We pass this in from the API directly
    im = user_img
    ...
    # mask of the image to remove background
    #output_path = "mask_output_{x}.png"
    #cv2.imwrite(output_path, matte)

    # return png without background with mask applied
    im_PIL = Image.fromarray(user_img)
    matte = Image.fromarray(matte)
    im_PIL.putalpha(matte)   # add alpha channel to keep transparency
    #plt.imshow(im_PIL)
    #plt.show()
    #im_PIL.save('detected.png')
    return im_PIL # Final Image